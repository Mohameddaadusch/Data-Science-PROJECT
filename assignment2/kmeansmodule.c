...
double **run(int k, int max_iter, double epsilon, int d, int num_vectors, double **vectors, double **init_centroids);
...
{
    double **vMatrix, **starting, **result;
    ...
    result = run(k, max_iter, epsilon, d, N, vMatrix, starting);
    if (result == NULL)
    {
        PyErr_SetString(PyExc_MemoryError, "An Error Has Occurred");
        return PyList_New(0);
...
static PyObject *fit_api(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "iidiOO", &k, &max_iter, &epsilon, &d, &vectors, &init_centroids))
    {
        return NULL;
    }
...
double help(double *vector1, double *vector2, int vDim)
{ /* help func for calculating the norm*/
    double *arr;
    int i;
    double sum;
